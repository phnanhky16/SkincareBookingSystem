...
const useBookingListCancelledHook = () => {
  ...
  // Fetch cancelled bookings when userId is available
  useEffect(() => {
    if (userId) {
      fetchCancelledBookings(userId);
    }
  }, [userId]);

  const fetchCancelledBookings = async (id) => {
    try {
      setLoading(true);
      setError(null);

      console.log("Fetching cancelled bookings for user:", id);
      ...
      const response = await APIClient.invoke({
        action: ACTIONS.GET_CUSTOMER_CANCELLED_BOOKINGS,
        pathParams: { userId: id },
        headers: authHeaders,
        options: { preventRedirect: true },
      });

      console.log("Cancelled bookings response:", response);
      ...
      if (response && response.success && response.result) {
        ...
        if (response && !response.success) {
          setError(response.message || "Failed to fetch cancelled bookings");
        }
      }
    } catch (err) {
  ...
  const refreshCancelledBookings = () => {
    if (userId) {
      fetchCancelledBookings(userId);
    }
  };
...
export default useBookingListCancelledHook; 